...
inline NxF32 fmi_computePlane(const NxF32 *A, const NxF32 *B, const NxF32 *C,
                              NxF32 *n) // returns D
{
  NxF32 vx = (B[0] - C[0]);
  NxF32 vy = (B[1] - C[1]);
...
inline void fmi_transform(const NxF32 matrix[16], const NxF32 v[3],
                          NxF32 t[3]) // rotate and translate this point
{
  if (matrix) {
    NxF32 tx = (matrix[0 * 4 + 0] * v[0]) + (matrix[1 * 4 + 0] * v[1]) +
               (matrix[2 * 4 + 0] * v[2]) + matrix[3 * 4 + 0];
    NxF32 ty = (matrix[0 * 4 + 1] * v[0]) + (matrix[1 * 4 + 1] * v[1]) +
               (matrix[2 * 4 + 1] * v[2]) + matrix[3 * 4 + 1];
    NxF32 tz = (matrix[0 * 4 + 2] * v[0]) + (matrix[1 * 4 + 2] * v[1]) +
               (matrix[2 * 4 + 2] * v[2]) + matrix[3 * 4 + 2];
    t[0] = tx;
    t[1] = ty;
    t[2] = tz;
  } else {
...
inline void fmi_transformRotate(const NxF32 matrix[16], const NxF32 v[3],
                                NxF32 t[3]) // rotate only
{
  if (matrix) {
    NxF32 tx = (matrix[0 * 4 + 0] * v[0]) + (matrix[1 * 4 + 0] * v[1]) +
               (matrix[2 * 4 + 0] * v[2]);
...
inline void
fmi_quatToMatrix(const NxF32 *quat,
                 NxF32 *matrix) // convert quaterinion rotation to matrix, zeros
                                // out the translation component.
{
...
inline void fmi_setOrientationFromAxisAngle(const NxF32 axis[3], NxF32 angle,
                                            NxF32 *quat) {
  NxF32 x, y, z, w;
...
inline void fmi_matrixToQuat(const NxF32 *matrix,
                             NxF32 *quat) // convert the 3x3 portion of a 4x4
                                          // matrix into a quaterion as x,y,z,w
{
...
inline void fmi_decomposeTransform(const NxF32 local_transform[16],
                                   NxF32 trans[3], NxF32 rot[4],
                                   NxF32 scale[3]) {
  ...
  scale[0] = sqrtf(fmi_squared(local_transform[0 * 4 + 0]) +
                   fmi_squared(local_transform[0 * 4 + 1]) +
                   fmi_squared(local_transform[0 * 4 + 2]));
  scale[1] = sqrtf(fmi_squared(local_transform[1 * 4 + 0]) +
                   fmi_squared(local_transform[1 * 4 + 1]) +
                   fmi_squared(local_transform[1 * 4 + 2]));
  scale[2] = sqrtf(fmi_squared(local_transform[2 * 4 + 0]) +
                   fmi_squared(local_transform[2 * 4 + 1]) +
                   fmi_squared(local_transform[2 * 4 + 2]));
...
inline void fmi_multiply(const NxF32 *pA, const NxF32 *pB, NxF32 *pM) {
  ...
  NxF32 a = pA[0 * 4 + 0] * pB[0 * 4 + 0] + pA[0 * 4 + 1] * pB[1 * 4 + 0] +
            pA[0 * 4 + 2] * pB[2 * 4 + 0] + pA[0 * 4 + 3] * pB[3 * 4 + 0];
  NxF32 b = pA[0 * 4 + 0] * pB[0 * 4 + 1] + pA[0 * 4 + 1] * pB[1 * 4 + 1] +
            pA[0 * 4 + 2] * pB[2 * 4 + 1] + pA[0 * 4 + 3] * pB[3 * 4 + 1];
  NxF32 c = pA[0 * 4 + 0] * pB[0 * 4 + 2] + pA[0 * 4 + 1] * pB[1 * 4 + 2] +
            pA[0 * 4 + 2] * pB[2 * 4 + 2] + pA[0 * 4 + 3] * pB[3 * 4 + 2];
  NxF32 d = pA[0 * 4 + 0] * pB[0 * 4 + 3] + pA[0 * 4 + 1] * pB[1 * 4 + 3] +
            pA[0 * 4 + 2] * pB[2 * 4 + 3] + pA[0 * 4 + 3] * pB[3 * 4 + 3];

  NxF32 e = pA[1 * 4 + 0] * pB[0 * 4 + 0] + pA[1 * 4 + 1] * pB[1 * 4 + 0] +
            pA[1 * 4 + 2] * pB[2 * 4 + 0] + pA[1 * 4 + 3] * pB[3 * 4 + 0];
  NxF32 f = pA[1 * 4 + 0] * pB[0 * 4 + 1] + pA[1 * 4 + 1] * pB[1 * 4 + 1] +
            pA[1 * 4 + 2] * pB[2 * 4 + 1] + pA[1 * 4 + 3] * pB[3 * 4 + 1];
  NxF32 g = pA[1 * 4 + 0] * pB[0 * 4 + 2] + pA[1 * 4 + 1] * pB[1 * 4 + 2] +
            pA[1 * 4 + 2] * pB[2 * 4 + 2] + pA[1 * 4 + 3] * pB[3 * 4 + 2];
  NxF32 h = pA[1 * 4 + 0] * pB[0 * 4 + 3] + pA[1 * 4 + 1] * pB[1 * 4 + 3] +
            pA[1 * 4 + 2] * pB[2 * 4 + 3] + pA[1 * 4 + 3] * pB[3 * 4 + 3];

  NxF32 i = pA[2 * 4 + 0] * pB[0 * 4 + 0] + pA[2 * 4 + 1] * pB[1 * 4 + 0] +
            pA[2 * 4 + 2] * pB[2 * 4 + 0] + pA[2 * 4 + 3] * pB[3 * 4 + 0];
  NxF32 j = pA[2 * 4 + 0] * pB[0 * 4 + 1] + pA[2 * 4 + 1] * pB[1 * 4 + 1] +
            pA[2 * 4 + 2] * pB[2 * 4 + 1] + pA[2 * 4 + 3] * pB[3 * 4 + 1];
  NxF32 k = pA[2 * 4 + 0] * pB[0 * 4 + 2] + pA[2 * 4 + 1] * pB[1 * 4 + 2] +
            pA[2 * 4 + 2] * pB[2 * 4 + 2] + pA[2 * 4 + 3] * pB[3 * 4 + 2];
  NxF32 l = pA[2 * 4 + 0] * pB[0 * 4 + 3] + pA[2 * 4 + 1] * pB[1 * 4 + 3] +
            pA[2 * 4 + 2] * pB[2 * 4 + 3] + pA[2 * 4 + 3] * pB[3 * 4 + 3];

  NxF32 m = pA[3 * 4 + 0] * pB[0 * 4 + 0] + pA[3 * 4 + 1] * pB[1 * 4 + 0] +
            pA[3 * 4 + 2] * pB[2 * 4 + 0] + pA[3 * 4 + 3] * pB[3 * 4 + 0];
  NxF32 n = pA[3 * 4 + 0] * pB[0 * 4 + 1] + pA[3 * 4 + 1] * pB[1 * 4 + 1] +
            pA[3 * 4 + 2] * pB[2 * 4 + 1] + pA[3 * 4 + 3] * pB[3 * 4 + 1];
  NxF32 o = pA[3 * 4 + 0] * pB[0 * 4 + 2] + pA[3 * 4 + 1] * pB[1 * 4 + 2] +
            pA[3 * 4 + 2] * pB[2 * 4 + 2] + pA[3 * 4 + 3] * pB[3 * 4 + 2];
  NxF32 p = pA[3 * 4 + 0] * pB[0 * 4 + 3] + pA[3 * 4 + 1] * pB[1 * 4 + 3] +
            pA[3 * 4 + 2] * pB[2 * 4 + 3] + pA[3 * 4 + 3] * pB[3 * 4 + 3];
...
inline void fmi_composeTransform(const NxF32 pos[3], const NxF32 quat[4],
                                 const NxF32 scale[3], NxF32 matrix[16]) {
  NxF32 mscale[16];
  NxF32 mrot[16];
...
inline void fmi_getSubMatrix(NxI32 ki, NxI32 kj, NxF32 pDst[16],
                             const NxF32 matrix[16]) {
  NxI32 row, col;
...
inline void fmi_inverseTransform(const NxF32 matrix[16],
                                 NxF32 inverse_matrix[16]) {
...
enum MeshVertexFlag {
  ...
  MIVF_INTERP7 = (1 << 17),
  MIVF_INTERP8 = (1 << 18),
  MIVF_ALL = (MIVF_POSITION | MIVF_NORMAL | MIVF_COLOR | MIVF_TEXEL1 |
              MIVF_TEXEL2 | MIVF_TEXEL3 | MIVF_TEXEL4 | MIVF_TANGENT |
              MIVF_BINORMAL | MIVF_BONE_WEIGHTING)
};
...
public:
  ...
  void Set(const char *name, NxI32 parentIndex, const NxF32 pos[3],
           const NxF32 rot[4], const NxF32 scale[3]) {
    mName = name;
...
public:
  ...
  void
  SetBones(NxI32 bcount,
           MeshBone *bones) // memory ownership changes hands here!!!!!!!!!!
  {
    mBoneCount = bcount;
    mBones = bones;
  }
...
public:
  ...
  void SampleAnimation(NxI32 frame, NxF32 *pos, NxF32 *quat,
                       NxF32 *scale) const {
...
public:
  ...
  void SetTrackPose(NxI32 track, NxI32 frame, const NxF32 *pos,
                    const NxF32 *quat, const NxF32 *scale) {
...
#define MESH_SYSTEM_VERSION                                                    \
  1 // version number of this data structure, used for binary serialization
...
public:
  MeshCollisionConvex(void) { mType = MCT_CONVEX; }
};
...
public:
  ...
  MeshRawTexture **mTextures; // Texture storage in mesh data is rare, and the
                              // name is simply an attribute of the material
  ...
  NxU32 mMaterialCount; // Materials are owned by this list, merely referenced
                        // later.
...
public:
  virtual void importMaterial(const char *matName,
                              const char *metaData) = 0; // one material
  virtual void importUserData(
      const char *userKey,
      const char
          *userValue) = 0; // carry along raw user data as ASCII strings only..
  virtual void importUserBinaryData(const char *name, NxU32 len,
                                    const NxU8 *data) = 0;
  virtual void importTetraMesh(const char *tetraName, const char *meshName,
                               NxU32 tcount, const NxF32 *tetraData) = 0;

  virtual void
  importAssetName(const char *assetName,
                  const char *info) = 0; // name of the overall asset.
  virtual void importMesh(
      const char *meshName,
      const char
          *skeletonName) = 0; // name of a mesh and the skeleton it refers to.

  virtual void importTriangle(const char *meshName, const char *materialName,
                              NxU32 vertexFlags, const MeshVertex &v1,
                              const MeshVertex &v2, const MeshVertex &v3) = 0;

  virtual void importIndexedTriangleList(const char *meshName,
                                         const char *materialName,
                                         NxU32 vertexFlags, NxU32 vcount,
  ...
  virtual void importRawTexture(const char *textureName, const NxU8 *pixels,
                                NxU32 wid, NxU32 hit) = 0;
  virtual void importMeshInstance(const char *meshName, const NxF32 pos[3],
                                  const NxF32 rotation[4],
                                  const NxF32 scale[3]) = 0;

  virtual void importCollisionRepresentation(
      const char *name,
      const char *info) = 0; // the name of a new collision representation.

  virtual void importConvexHull(
      const char
          *collision_rep, // the collision representation it is associated with
      const char *
          boneName, // the name of the bone it is associated with in a skeleton.
      const NxF32 *transform, // the full 4x4 transform for this hull, null if
                              // in world space.
      NxU32 vertex_count, const NxF32 *vertices, NxU32 tri_count,
      const NxU32 *indices) = 0;

  virtual void importSphere(
      const char
          *collision_rep, // the collision representation it is associated with
      const char *
          boneName, // the name of the bone it is associated with in a skeleton.
      const NxF32 *transform, NxF32 radius) = 0;

  virtual void importCapsule(
      const char
          *collision_rep, // the collision representation it is associated with
      const char *
          boneName, // the name of the bone it is associated with in a skeleton.
      const NxF32 *transform, // the full 4x4 transform for this hull, null if
                              // in world space.
      NxF32 radius, NxF32 height) = 0;

  virtual void importOBB(
      const char
          *collision_rep, // the collision representation it is associated with
      const char *
          boneName, // the name of the bone it is associated with in a skeleton.
...
// For example, when loading wavefront OBJ files, the materials are saved in a
// seperate file. This interface allows the application to load the resource.
class MeshImportApplicationResource {
public:
  virtual void *getApplicationResource(const char *base_name,
...
class MeshImporter {
public:
  virtual NxI32 getExtensionCount(void) {
    return 1;
  }; // most importers support just one file name extension.
  virtual const char *getExtension(
      NxI32 index =
          0) = 0; // report the default file name extension for this mesh type.
  virtual const char *getDescription(
      NxI32 index = 0) = 0; // report the ascii description of the import type.

  virtual bool importMesh(const char *meshName, const void *data, NxU32 dlen,
                          MeshImportInterface *callback, const char *options,
                          MeshImportApplicationResource *appResource) = 0;

  virtual const void *saveMeshSystem(MeshSystem * /*ms*/, NxU32 & /*dlen*/,
                                     bool /*binary*/) {
    return NULL;
  }
...
public:
  MeshBoneInstance(void) { mBoneName = ""; }
  ...
  NxF32 mAnimTransform[16]; // the sampled animation transform, multiplied times
...
public:
  ...
  NxU8 *mExtendedData;
  NxU32 mExtendedLen;
  const char *mSaveFileName;  // need to know the name of the save file name for
                              // OBJ and Ogre3d format.
...
public:
  virtual NxU32
  getIndex(const NxF32 pos[3],
           bool &newPos) = 0; // get welded index for this NxF32 vector[3]
...
public:
  virtual void
  addImporter(MeshImporter *importer) = 0; // add an additional importer

  virtual bool importMesh(const char *meshName, const void *data, NxU32 dlen,
                          MeshImportInterface *callback,
                          const char *options) = 0;

  virtual MeshSystemContainer *createMeshSystemContainer(void) = 0;

  virtual MeshSystemContainer *createMeshSystemContainer(
      const char *meshName, const void *data, NxU32 dlen,
      const char *options) = 0; // imports and converts to a single MeshSystem
                                // data structure
  ...
  locateMeshImporter(const char *fname) = 0; // based on this file name, find a
                                             // matching mesh importer.

  virtual const char *getFileRequestDialogString(void) = 0;

  virtual void
  setMeshImportApplicationResource(MeshImportApplicationResource *resource) = 0;

  // convenience helper functions.
  virtual MeshSkeletonInstance *
  createMeshSkeletonInstance(const MeshSkeleton &sk) = 0;
  virtual bool sampleAnimationTrack(NxI32 trackIndex, const MeshSystem *msystem,
  ...
  virtual MeshImportInterface *
  getMeshImportInterface(MeshSystemContainer *msc) = 0;

  virtual void gather(MeshSystemContainer *msc) = 0;

  virtual void scale(MeshSystemContainer *msc, NxF32 scale) = 0;
  virtual void rotate(MeshSystemContainer *msc, NxF32 rotX, NxF32 rotY,
                      NxF32 rotZ) = 0; // rotate mesh system using these euler
                                       // angles expressed as degrees.

  virtual VertexIndex *createVertexIndex(
      NxF32 granularity) = 0; // create an indexed vertext system for floats
...
#define MESHIMPORT_VERSION                                                     \
  11 // version 0.01  increase this version number whenever an interface change
     // occurs.

extern MeshImport
    *gMeshImport; // This is an optional global variable that can be used by the
                  // application.  If the application uses it, it should define
                  // it somewhere in its codespace.

MeshImport *loadMeshImporters(
    const char *directory); // loads the mesh import library (dll) and all
                            // available importers from the same directory.
