...
static void *getMeshBindingInterface(const char *dll, NxI32 version_number) {
  void *ret = 0;
...
void GetFileList(std::vector<std::string> &list, std::string dir,
                 std::string extension, std::string filenameToIgnore) {
...
NVSHARE::MeshImport *loadMeshImporters(
...
#ifdef WIN32
  ret = (NVSHARE::MeshImport *)getMeshBindingInterface(scratch,
                                                       MESHIMPORT_VERSION);
#else
  ret = 0;
#endif
...
#ifdef WIN32
      NVSHARE::MeshImporter *imp =
          (NVSHARE::MeshImporter *)getMeshBindingInterface(plugins[i].c_str(),
                                                           MESHIMPORT_VERSION);
#else
