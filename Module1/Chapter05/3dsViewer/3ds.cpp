...
bool C3dsLoader::Load3DS(const std::string &filename,
  ...
  while (infile.tellg() < fileSize) {
    ...
    case 0x4140: {
      ...
      infile.read(reinterpret_cast<char *>(&pMesh->uvs[0].x),
                  sizeof(glm::vec2) * total_uvs);
    } break;
    ...
    case 0x4160: {
      ...
      infile.read(reinterpret_cast<char *>(&transform[0][0]),
                  12 * sizeof(float));

      pMesh->transform =
          glm::mat4(transform[0][0], transform[0][1], transform[0][2], 0,
    ...
    case 0xa34C: // Mask for reflection map
    {
      ...
      pCurrentTextureMap =
          pMaterial->textureMaps[pMaterial->textureMaps.size() - 1];
    } break;
    ...
    case 0xa353: // Blur percent
      infile.read(reinterpret_cast<char *>(&pCurrentTextureMap->blur_percent),
                  sizeof(float));
      break;

    case 0xa354: // V scale
      infile.read(reinterpret_cast<char *>(&pCurrentTextureMap->UVscale[1]),
                  sizeof(float));
      break;

    case 0xa356: // U scale
      infile.read(reinterpret_cast<char *>(&pCurrentTextureMap->UVscale[0]),
                  sizeof(float));
      break;

    case 0xa358: // U offset
      infile.read(reinterpret_cast<char *>(&pCurrentTextureMap->UVoffset[0]),
                  sizeof(float));
      break;

    case 0xa35A: // V offset
      infile.read(reinterpret_cast<char *>(&pCurrentTextureMap->UVoffset[1]),
                  sizeof(float));
      break;

    case 0xa35C: // Rotation angle
      infile.read(reinterpret_cast<char *>(&pCurrentTextureMap->rotation_angle),
                  sizeof(float));
      break;

    case 0xa360: // RGB Luma/Alpha tint 1
      infile.read(
          reinterpret_cast<char *>(&pCurrentTextureMap->rgbLumAlphaTint1[0]),
          3 * sizeof(float));
      break;

    case 0xa362: // RGB Luma/Alpha tint 2
      infile.read(
          reinterpret_cast<char *>(&pCurrentTextureMap->rgbLumAlphaTint2[0]),
          3 * sizeof(float));
      break;
  ...
  // check if there is any material with 0 size face_ids meaning it is not used
  // then delete it
