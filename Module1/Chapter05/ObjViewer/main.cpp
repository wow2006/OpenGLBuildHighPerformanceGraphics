...
// Objloader instance
ObjLoader obj;
std::vector<Mesh *> meshes;          // all meshes
std::vector<Material *> materials;   // all materials
std::vector<unsigned short> indices; // all mesh indices
std::vector<Vertex> vertices;        // all mesh vertices
...
void OnInit() {
  ...
  for (size_t k = 0; k < materials.size(); k++) {
    ...
    if (materials[k]->map_Kd != "") {
      ...
      // allocate texture
      glTexImage2D(GL_TEXTURE_2D, 0, static_cast<int>(format), texture_width, texture_height, 0,
  ...
  glVertexAttribPointer(
      shader["vNormal"], 3, GL_FLOAT, GL_FALSE, sizeof(Vertex),
...
void OnResize(int w, int h) {
  ...
  P = glm::perspective(60.0f, static_cast<float>(w) / static_cast<float>(h),
                       0.1F, 1000.0F);
}
...
void OnRender() {
  ...
  const auto T = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, dist));
  const auto Rx = glm::rotate(T, rX, glm::vec3(1.0f, 0.0f, 0.0f));
  const auto MV = glm::rotate(Rx, rY, glm::vec3(0.0f, 1.0f, 0.0f));
  ...
  glBindVertexArray(vaoID);
  {
    ...
    for (size_t i = 0; i < materials.size(); i++) {
      ...
      if (pMat->map_Kd != "") {
        ...
        if (static_cast<uint32_t>(whichID[0]) != textures[i])
      ...
      if (materials.size() == 1)
        glDrawElements(GL_TRIANGLES, static_cast<int>(indices.size()), GL_UNSIGNED_SHORT, 0);
      else
        // otherwise we render the submesh
        glDrawElements(GL_TRIANGLES, pMat->count, GL_UNSIGNED_SHORT,
                       reinterpret_cast<const GLvoid *>(&indices[static_cast<size_t>(pMat->offset)]));
    }
    // unbind the shader
    shader.UnUse();
  }
  ...
  glBindVertexArray(lightVAOID);
  {
    ...
    glm::mat4 Translation = glm::translate(glm::mat4(1), lightPosOS);
    // bind the shader
    flatShader.Use();
    // set shader uniforms and draw lines
    glUniformMatrix4fv(flatShader("MVP"), 1, GL_FALSE,
                       glm::value_ptr(P * MV * Translation));
...
void OnMouseMove(int x, int y) {
  if (state == 0)
    dist *= (1.F + static_cast<float>(y - oldY) / 60.0f);
  else if (state == 2) {
    theta += static_cast<float>(oldX - x) / 60.0f;
    phi += static_cast<float>(y - oldY) / 60.0f;
    ...
    rY += static_cast<float>(x - oldX) / 5.0f;
    rX += static_cast<float>(y - oldY) / 5.0f;
  }
  oldX = x;
  oldY = y;
