...
glm::vec4 box_colors[3] = {glm::vec4(1, 0, 0, 0.5), glm::vec4(0, 1, 0, 0.5),
                           glm::vec4(0, 0, 1, 0.5)};
...
// shaders for cube, initialization, dual depth peeling, blending and final
// rendering
...
// and a shader
void DrawScene(const glm::mat4 &MVP, GLSLShader &shader, bool useColor = false,
               bool useAlphaMultiplier = false);
...
void initFBO() {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    glTexImage2D(GL_TEXTURE_RECTANGLE, 0, GL_FLOAT_RG32_NV, WIDTH, HEIGHT, 0,
  ...
  glTexParameteri(GL_TEXTURE_RECTANGLE, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
  glTexParameteri(GL_TEXTURE_RECTANGLE, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
  glTexImage2D(GL_TEXTURE_RECTANGLE, 0, GL_RGB, WIDTH, HEIGHT, 0, GL_RGB,
               GL_FLOAT, 0);
  ...
  for (int i = 0; i < 2; i++) {
    glFramebufferTexture2D(GL_FRAMEBUFFER, attachID[i], GL_TEXTURE_RECTANGLE,
                           depthTexID[i], 0);
...
void DrawScene(const glm::mat4 &MVP, GLSLShader &shader, bool useColor,
               bool useAlphaMultiplier) {
