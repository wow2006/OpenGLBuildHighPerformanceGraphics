...
struct Common {
  ...
  glm::vec4 mBox_colors[3] = {glm::vec4(1,0,0,0.5),
               glm::vec4(0,1,0,0.5),
               glm::vec4(0,0,1,0.5)
                };
...
void initFBO() {
  ...
  for(int i=0;i<2;i++) {
    ...
    glTexImage2D(GL_TEXTURE_RECTANGLE , 0, GL_DEPTH_COMPONENT32F,
        Common::WIDTH, Common::HEIGHT, 0, GL_DEPTH_COMPONENT,
        GL_FLOAT, nullptr);
...
void OnInit() {
  ...
  g_pCommon->m_pGrid = new CGrid(20,20);
	...
	g_pCommon->mCubeShader.LoadFromFile(GL_VERTEX_SHADER,   "shaders/cube_shader.vert");
	g_pCommon->mCubeShader.LoadFromFile(GL_FRAGMENT_SHADER, "shaders/cube_shader.frag");
	...
	g_pCommon->mFrontPeelShader.LoadFromFile(GL_VERTEX_SHADER,   "shaders/front_peel.vert");
	g_pCommon->mFrontPeelShader.LoadFromFile(GL_FRAGMENT_SHADER, "shaders/front_peel.frag");
	//compile and link the shader
	...
	g_pCommon->mBlendShader.LoadFromFile(GL_FRAGMENT_SHADER, "shaders/blend.frag");
	//compile and link the shader
	...
	g_pCommon->mFinalShader.LoadFromFile(GL_FRAGMENT_SHADER, "shaders/final.frag");
	//compile and link the shader
	...
	for(int k=-1;k<=1;k++) {
		for(int j=-1;j<=1;j++) {
			...
			for(int i=-1;i<=1;i++) {
				...
				glUniformMatrix4fv(shader("MVP"), 1, GL_FALSE, glm::value_ptr(MVP * g_pCommon->mR * T));
				//draw the cube
...
void OnRender() {
	...
	auto Tr	= glm::translate(glm::mat4(1.0f),glm::vec3(0.0f, 0.0f, g_pCommon->mDist));
	...
	if(g_pCommon->m_bShowDepthPeeling) {
		...
		for (int layer = 1; g_pCommon->m_bUseOQ || layer < numLayers; layer++) {
			...
			if(g_pCommon->m_bUseOQ) {
				GLuint sample_count;
				glGetQueryObjectuiv(g_pCommon->mQueryId, GL_QUERY_RESULT, &sample_count);
				if (sample_count == 0) {
					break;
				}
			}
...
void OnResize(int w, int h) {
	...
	g_pCommon->mP = glm::perspective(45.0f, static_cast<float>(w) / h,
                                   0.1f, 1000.0f);
}
...
void OnIdle() {
	...
	g_pCommon->mR = glm::rotate(glm::mat4(1), glm::radians(g_pCommon->mAngle += 5),
                              glm::vec3(0,1,0));
