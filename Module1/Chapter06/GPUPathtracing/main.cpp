...
void OnInit() {
  ...
  if (!obj.Load(mesh_filename.c_str(), meshes, vertices, indices, materials,
    ...
    std::cout << "Cannot load the 3ds mesh" << endl;
    exit(EXIT_FAILURE);
  }
  GL_CHECK_ERRORS;
  ...
  for (size_t k = 0; k < materials.size(); k++) {
    ...
    if (materials[k]->map_Kd != "") {
      ...
      if (pData == NULL) {
        std::cerr << "Cannot load image: " << full_filename.c_str() << endl;
        exit(EXIT_FAILURE);
      }
  ...
  glUniform1f(pathtraceShader("TRIANGLE_TEXTURE_SIZE"),
  ...
  if (materials.size() == 1) {
    // pass indices to the element array buffer if there is a single
    // material
  ...
  if (bPathtrace) {
    ...
    glBindVertexArray(vaoID);
    {
      ...
      for (size_t i = 0; i < materials.size(); i++) {
        ...
        // if we have a single material, we render the whole mesh in a
        // single call
        if (materials.size() == 1)
...
void OnShutdown() {
  ...
  std::cout << "Shutdown successfull" << endl;
}
