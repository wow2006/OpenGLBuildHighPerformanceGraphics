...
const std::string mesh_filename = "GPURaytracingData/media/blocks.obj";
...
auto main(int argc, char *argv[]) -> int {
  ...
  glutMouseWheelFunc(Mouse::OnMouseWheel);
...
void OnInit() {
  ...
  // load flat shader
  flatShader.LoadFromFile(GL_VERTEX_SHADER, "GPURaytracingData/shaders/flat.vert");
  flatShader.LoadFromFile(GL_FRAGMENT_SHADER, "GPURaytracingData/shaders/flat.frag");
  ...
  raytraceShader.LoadFromFile(GL_VERTEX_SHADER, "GPURaytracingData/shaders/raytracer.vert");
  raytraceShader.LoadFromFile(GL_FRAGMENT_SHADER, "GPURaytracingData/shaders/raytracer.frag");
  // compile and link shader
  ...
  glUniform1f(raytraceShader("TRIANGLE_TEXTURE_SIZE"),
              (float)indices2.size() / 4);
  ...
  shader.LoadFromFile(GL_VERTEX_SHADER, "GPURaytracingData/shaders/shader.vert");
  shader.LoadFromFile(GL_FRAGMENT_SHADER, "GPURaytracingData/shaders/shader.frag");
  // compile and link shader
...
void OnRender() {
  ...
  if (bRaytrace) {
    ...
    glUniformMatrix4fv(raytraceShader("invMVP"), 1, GL_FALSE,
    ...
    glBindVertexArray(vaoID);
    {
      ...
      for (size_t i = 0; i < materials.size(); i++) {
        ...
        // if we have a single material, we render the whole mesh in a single
        // call
        if (materials.size() == 1)
