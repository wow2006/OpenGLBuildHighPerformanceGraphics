...
CQuad::CQuad(float zpos, const char* vert, const char* frag) {
  ...
  *id++ = 0; 	*id++ = 2; 	*id++ = 3;
  ...
  glBindVertexArray(vaoID);
          ...
          glBufferData (GL_ARRAY_BUFFER, 8*sizeof(glm::vec3), &vertices[0],
  GL_STATIC_DRAW);
          ...
          glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLushort)*total_indices,
...
void CQuad::Render(const float* MVP) {
        ...
        glBindVertexArray(vaoID);
                glDrawElements(GL_TRIANGLES, total_indices, GL_UNSIGNED_SHORT,
0); glBindVertexArray(0); shader.UnUse();
} */
int CQuad::GetTotalVertices() { return 4; }

int CQuad::GetTotalIndices() { return 6; }

GLenum CQuad::GetPrimitiveType() { return GL_TRIANGLES; }
