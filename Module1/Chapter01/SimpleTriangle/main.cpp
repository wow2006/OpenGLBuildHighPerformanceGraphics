...
void OnInit() {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(g_pCommon->vertices),
               &g_pCommon->vertices[0], GL_STATIC_DRAW);
  GL_CHECK_ERRORS
  // enable vertex attribute array for position
  glEnableVertexAttribArray(g_pCommon->shader["vVertex"]);
  glVertexAttribPointer(g_pCommon->shader["vVertex"], 3, GL_FLOAT, GL_FALSE,
                        stride, nullptr);
  GL_CHECK_ERRORS
  // enable vertex attribute array for colour
  glEnableVertexAttribArray(g_pCommon->shader["vColor"]);
  glVertexAttribPointer(
      g_pCommon->shader["vColor"], 3, GL_FLOAT, GL_FALSE, stride,
      reinterpret_cast<const GLvoid *>(offsetof(Common::Vertex, color)));
  GL_CHECK_ERRORS
  // pass indices to element array buffer
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, g_pCommon->vboIndicesID);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(g_pCommon->indices),
...
void OnRender() {
  ...
  glUniformMatrix4fv(g_pCommon->shader("MVP"), 1, GL_FALSE,
                     glm::value_ptr(g_pCommon->P * g_pCommon->MV));
  // drwa triangle
