...
const glm::vec4 jet_values[9] = {
    glm::vec4(0, 0, 0.5, 0),      glm::vec4(0, 0, 1, 0.1),
    glm::vec4(0, 0.5, 1, 0.3),    glm::vec4(0, 1, 1, 0.5),
    glm::vec4(0.5, 1, 0.5, 0.75), glm::vec4(1, 1, 0, 0.8),
    glm::vec4(1, 0.5, 0, 0.6),    glm::vec4(1, 0, 0, 0.5),
...
// Function that load a volume from the given raw data file and generates an
// OpenGL 3D texture from it
bool LoadVolume();

// Function to generate interpolated colours from the set of colour values
// (jet_values) this function first calculates the amount of increments for each
// component and the index difference. Then it linearly interpolates the
...
void OnKey(unsigned char key, int x, int y) {
  ...
  num_slices = std::min<int>(MAX_SLICES, std::max<int>(num_slices, 3));
...
void OnInit() {
  ...
  // load the transfer function data and generate the trasnfer function (lookup
  // table) texture
...
void OnRender() {
  ...
  glDrawArrays(GL_TRIANGLES, 0,
               sizeof(vTextureSlices) / sizeof(vTextureSlices[0]));
  // unbind the shader
  shader.UnUse();
...
void SliceVolume() {
  // get the max and min distance of each vertex of the unit cube in the viewing
  // direction
  ...
  for (int i = 1; i < 8; i++) {
    // get the distance between the current unit cube vertex and the view vector
    // by dot product
  ...
  // local variables to store the start, direction vectors, lambda intersection
  // values
  ...
  for (int i = num_slices - 1; i >= 0; i--) {
    ...
    // if the values are between 0-1, we have an intersection at the current
    // edge repeat the same for all 12 edges
    ...
    // we calculated the proper polygon indices by using indices of a triangular
    // fan
    int indices[] = {0, 1, 2, 0, 2, 3, 0, 3, 4, 0, 4, 5};

    // using the indices, pass the intersection vertices to the vTextureSlices
    // vector
    for (int i = 0; i < 12; i++)
...
bool LoadVolume() {
  ...
  if (infile.good()) {
    ...
    glTexImage3D(GL_TEXTURE_3D, 0, GL_RED, XDIM, YDIM, ZDIM, 0, GL_RED,
...
void LoadTransferFunction() {
  ...
  // fill the colour values at the place where the colour should be after
  // interpolation
  for (int i = 0; i < 9; i++) {
    int index = i * 28;
  ...
  // allocate the data to texture memory. Since pData is on stack, we donot
  // delete it
