...
void OnMouseMove(int x, int y) {
  if (g_pCommon->state == 0) {
    ...
    g_pCommon->lightPosOS.x = g_pCommon->radius * std::cos(g_pCommon->theta) *
                              std::sin(g_pCommon->phi);
    g_pCommon->lightPosOS.y = g_pCommon->radius * std::cos(g_pCommon->phi);
    g_pCommon->lightPosOS.z = g_pCommon->radius * std::sin(g_pCommon->theta) *
                              std::sin(g_pCommon->phi);
...
void OnInit() {
  ...
  g_pCommon->gaussianH_shader.LoadFromFile(GL_VERTEX_SHADER,
                                           "shaders/Passthrough.vert");
  g_pCommon->gaussianH_shader.LoadFromFile(GL_FRAGMENT_SHADER,
                                           "shaders/GaussH.frag");
  // compile and link shader
  g_pCommon->gaussianH_shader.CreateAndLinkProgram();
  g_pCommon->gaussianH_shader.Use();
  // add attributes and uniforms
  g_pCommon->gaussianH_shader.AddAttribute("vVertex");
  g_pCommon->gaussianH_shader.AddUniform("textureMap");
  // pass value of constant uniforms at initialization
  glUniform1i(g_pCommon->gaussianH_shader("textureMap"), 1);
  g_pCommon->gaussianH_shader.UnUse();
  GL_CHECK_ERRORS;

  // load the vertical Gaussian smoothing shader
  g_pCommon->gaussianV_shader.LoadFromFile(GL_VERTEX_SHADER,
                                           "shaders/Passthrough.vert");
  g_pCommon->gaussianV_shader.LoadFromFile(GL_FRAGMENT_SHADER,
                                           "shaders/GaussV.frag");
  // compile and link shader
  g_pCommon->gaussianV_shader.CreateAndLinkProgram();
  g_pCommon->gaussianV_shader.Use();
  // add attributes and uniforms
  g_pCommon->gaussianV_shader.AddAttribute("vVertex");
  g_pCommon->gaussianV_shader.AddUniform("textureMap");
  // pass value of constant uniforms at initialization
  glUniform1i(g_pCommon->gaussianV_shader("textureMap"), 0);
  g_pCommon->gaussianV_shader.UnUse();
  GL_CHECK_ERRORS;

  // load the variance shadow mapping first step shader
  g_pCommon->firstStep.LoadFromFile(GL_VERTEX_SHADER, "shaders/firstStep.vert");
  g_pCommon->firstStep.LoadFromFile(GL_FRAGMENT_SHADER,
                                    "shaders/firstStep.frag");
  // compile and link shader
  g_pCommon->firstStep.CreateAndLinkProgram();
  g_pCommon->firstStep.Use();
  // add attributes and uniforms
  g_pCommon->firstStep.AddAttribute("vVertex");
  g_pCommon->firstStep.AddUniform("MVP");
  g_pCommon->firstStep.UnUse();
  GL_CHECK_ERRORS;

  // load the variance shadow mapping shader
  g_pCommon->shader.LoadFromFile(GL_VERTEX_SHADER,
                                 "shaders/VarianceShadowMapping.vert");
  g_pCommon->shader.LoadFromFile(GL_FRAGMENT_SHADER,
                                 "shaders/VarianceShadowMapping.frag");
  // compile and link shader
  ...
  // pass value of constant uniforms at initialization
  glUniform1i(g_pCommon->shader("shadowMap"), 2);
  ...
  // setup cube geometry
  CreateCube(2.f, g_pCommon->vertices, g_pCommon->indices);
  ...
  // pass quad indices
  g_pCommon->indices.clear();
  g_pCommon->indices.push_back(0);
  g_pCommon->indices.push_back(1);
  g_pCommon->indices.push_back(2);

  g_pCommon->indices.push_back(0);
  g_pCommon->indices.push_back(2);
  g_pCommon->indices.push_back(3);

  // generate quad vertex array and vertex buffer objects
  glGenVertexArrays(1, &g_pCommon->quadVAOID);
  glGenBuffers(1, &g_pCommon->quadVBOID);
  glGenBuffers(1, &g_pCommon->quadIndicesID);

  glBindVertexArray(g_pCommon->quadVAOID);
  glBindBuffer(GL_ARRAY_BUFFER, g_pCommon->quadVBOID);
  ...
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, g_pCommon->quadIndicesID);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               g_pCommon->indices.size() * sizeof(GLushort),
               &g_pCommon->indices[0], GL_STATIC_DRAW);
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, Common::SHADOWMAP_WIDTH,
               Common::SHADOWMAP_HEIGHT, 0, GL_RGBA, GL_FLOAT, nullptr);
  ...
  glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                            GL_RENDERBUFFER, g_pCommon->rboID);
  ...
  // setup filtering fbo
  glGenFramebuffers(1, &g_pCommon->filterFBOID);
  glBindFramebuffer(GL_FRAMEBUFFER, g_pCommon->filterFBOID);

  // generate two colour attachments on texture units 1 and 2
  glGenTextures(2, g_pCommon->blurTexID);
  for (int i = 0; i < 2; i++) {
    // set texture parameters
    glActiveTexture(GL_TEXTURE1 + i);
    glBindTexture(GL_TEXTURE_2D, g_pCommon->blurTexID[i]);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i,
                           GL_TEXTURE_2D, g_pCommon->blurTexID[i], 0);
  }
  // check the framebuffer completeness status
  status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
  if (status == GL_FRAMEBUFFER_COMPLETE) {
    std::cout << "Filtering FBO setup successful." << std::endl;
  } else {
    std::cout << "Problem in Filtering FBO setup." << std::endl;
  }
  ...
  g_pCommon->MV_L = glm::lookAt(g_pCommon->lightPosOS, glm::vec3(0, 0, 0),
                                glm::vec3(0, 1, 0));
  g_pCommon->P_L = glm::perspective(50.0f, 1.0f, 1.0f, 50.0f);
  g_pCommon->B =
...
void OnShutdown() {
  glDeleteTextures(1, &g_pCommon->shadowMapTexID);
  glDeleteTextures(2, g_pCommon->blurTexID);

  // Destroy shaders
  g_pCommon->shader.DeleteShaderProgram();
  g_pCommon->flatshader.DeleteShaderProgram();
  g_pCommon->firstStep.DeleteShaderProgram();
  g_pCommon->gaussianH_shader.DeleteShaderProgram();
  g_pCommon->gaussianV_shader.DeleteShaderProgram();
  ...
  glDeleteBuffers(1, &g_pCommon->planeVerticesVBO);
  glDeleteBuffers(1, &g_pCommon->planeIndicesVBO);
  glDeleteVertexArrays(1, &g_pCommon->planeVAOID);

  glDeleteVertexArrays(1, &g_pCommon->quadVAOID);
  glDeleteBuffers(1, &g_pCommon->quadVBOID);
  glDeleteBuffers(1, &g_pCommon->quadIndicesID);
  ...
  glDeleteFramebuffers(1, &g_pCommon->filterFBOID);
  glDeleteRenderbuffers(1, &g_pCommon->rboID);
...
void DrawSceneFirstPass(glm::mat4 View, glm::mat4 Proj) {
  ...
  {
    // set shader uniforms
    glUniformMatrix4fv(g_pCommon->firstStep("MVP"), 1, GL_FALSE,
...
void OnRender() {
  ...
  auto Rx = glm::rotate(T, g_pCommon->rX, glm::vec3(1.0f, 0.0f, 0.0f));
  ...
  DrawSceneFirstPass(g_pCommon->MV_L, g_pCommon->P_L);
...
void OnMouseWheel(int /*button*/, int dir, int /*x*/, int /*y*/) {
	if (dir > 0) {
    g_pCommon->radius += 0.1f;
  } else {
    g_pCommon->radius -= 0.1f;
  }

	g_pCommon->radius = std::max(g_pCommon->radius, 0.0f);
	...
	//update light's MV matrix
	g_pCommon->MV_L = glm::lookAt(g_pCommon->lightPosOS, glm::vec3(0,0,0), glm::vec3(0,1,0));
	g_pCommon->S = g_pCommon->BP * g_pCommon->MV_L;
