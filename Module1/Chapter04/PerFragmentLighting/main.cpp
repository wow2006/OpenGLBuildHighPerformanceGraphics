...
void OnInit() {
  ...
  g_pCommon->shader.LoadFromFile(GL_VERTEX_SHADER,   "shaders/perFragmentLighting.vert");
  g_pCommon->shader.LoadFromFile(GL_FRAGMENT_SHADER, "shaders/perFragmentLighting.frag");
...
void DrawScene(glm::mat4 View, glm::mat4 Proj) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    const float theta = i / 8.0f * 2.f * glm::pi<float>();
