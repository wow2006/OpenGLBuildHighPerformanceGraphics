...
void OnMouseDown(int button, int s, int x, int y) {
  if (s == GLUT_DOWN) {
    ...
    g_pCommon->oldY = y;
		...
		GLubyte pixel[4];
		glReadPixels(x, Common::HEIGHT-y, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, pixel);

		//based on colour decide which box was selected
    g_pCommon->selected_box = -1;
		if(pixel[0]==255 && pixel[1]==0 && pixel[2]==0) {
			std::cout<<"picked box 1"<<std::endl;
			g_pCommon->selected_box = 0;
		}
		if(pixel[0]==0 && pixel[1]==255 && pixel[2]==0) {
			std::cout<<"picked box 2"<<std::endl;
			g_pCommon->selected_box = 1;
		}
		if(pixel[0]==0 && pixel[1]==0 && pixel[2]==255) {
			std::cout<<"picked box 3"<<std::endl;
			g_pCommon->selected_box = 2;
		}
	}
